...
import { IRol, IUser } from "@/app/model/VentaModel";
import {
  obtenerEstadoCivil,
  obtenerRol,
  obtenerRoles,
...
const EditUser: React.FC<idCliente> = ({ idCliente }) => {
  const [defaultValues, setDefaultValues] = useState<TFormUsuario | null>(null);
  ...
  const onSubmit: SubmitHandler<TFormUsuario> = async (data) => {
    let persona: IPersona = {
      ...
      codEstado: usuario?.persona.codEstado ?? "",
      ...
      fecRegistro: usuario?.persona.fecRegistro ?? "",
    ...
    let role: IRol = {
      idRol: Number(data.codRol),
      nombreRol: (await obtenerRol(Number(data.codRol))).nombreRol,
    };

    let saveUser: IUser = {
      codEstado: usuario?.codEstado ?? "",
      emailUsuario: data.correoElectronico,
    ...
    try {
      const response = await actualizarUsuario(saveUser);
    ...
    const get = async () => {
      try {
        ...
        const formatForm: TFormUsuario = {
          ...
          codRol: response.roles[0].idRol.toString(),
          correoElectronico: response.emailUsuario.toString(),
        };
